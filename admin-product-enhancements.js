/* Origen Fit · mejoras de producto para admin: envío gratis + galería + texto promo + sabores */
document.addEventListener('DOMContentLoaded',()=>{
  ...
  if(checks && !$('pFlavorsEnabled')){
    const label=document.createElement('label');
    label.innerHTML='<input id="pFlavorsEnabled" type="checkbox"> Usar sabores';
    checks.appendChild(label);
  }
  if(checks && !$('pFlavorsBox')){
    const box=document.createElement('div');
    box.className='field';
    box.id='pFlavorsBox';
    box.style.display='none';
    box.innerHTML='<label>Sabores disponibles</label><textarea id="pFlavors" rows="5" placeholder="Chocolate\nVainilla\nFrutilla"></textarea><div class="muted" style="font-size:.82rem;margin-top:6px">Escribí un sabor por renglón. El cliente deberá elegir uno antes de agregar el producto al carrito.</div>';
  ...
  if(mainImageBox && !$('pGalleryBox')){
    ...
    style.textContent=`
      ...
      #pFlavors{width:100%;resize:vertical;min-height:112px}
    `;
  ...
  async function loadEnhancements(){
    ...
    if(!id){
      ...
      if(flavorsEnabled)flavorsEnabled.checked=false;
      if(flavorsInput)flavorsInput.value='';
      toggleFlavors();
      currentGallery=[];
    ...
    const [{data:p,error:pe},{data:g,error:ge}]=await Promise.all([
      db.from('products').select('free_shipping,promo_label,flavors_enabled,flavors').eq('id',id).single(),
    ...
    if(!pe){
      ...
      if(flavorsEnabled)flavorsEnabled.checked=!!p?.flavors_enabled;
      if(flavorsInput)flavorsInput.value=Array.isArray(p?.flavors)?p.flavors.join('\n'):'';
      toggleFlavors();
    }
  ...
  flavorsEnabled?.addEventListener('change',toggleFlavors);

  galleryPreview?.addEventListener('click',async e=>{
    const id=e.target?.dataset?.deleteGallery;if(!id)return;
    const row=currentGallery.find(x=>x.id===id);if(!row)return;
    e.target.disabled=true;
    try{
      if(row.storage_path)await db.storage.from('product-images').remove([row.storage_path]);
      const {error}=await db.from('product_images').delete().eq('id',id);if(error)throw error;
      currentGallery=currentGallery.filter(x=>x.id!==id);renderGallery();
    }catch(err){e.target.disabled=false;alert('No se pudo eliminar la foto: '+(err.message||err));}
  });

  const waitForCoreSave=()=>new Promise(resolve=>{
    const started=Date.now();
    const tick=()=>{
      const ok=$('pOk')?.textContent||'',err=$('pErr')?.textContent||'';
      if(/guardado correctamente/i.test(ok)||err||Date.now()-started>15000)resolve({ok,err});else setTimeout(tick,120);
    };
    setTimeout(tick,80);
  });
  ...
  $('pSave')?.addEventListener('click',async()=>{
    ...
    const desiredFlavorsEnabled=!!flavorsEnabled?.checked;
    const desiredFlavors=parseFlavors(flavorsInput?.value);
    const pending=[...(galleryInput?.files||[])];
    const outcome=await waitForCoreSave();
    if(outcome.err||!/guardado correctamente/i.test(outcome.ok))return;
    const productId=$('productId')?.value;if(!productId)return;

    try{
      const {error:updateError}=await db.from('products').update({free_shipping:desiredFreeShipping,promo_label:desiredPromoLabel,flavors_enabled:desiredFlavorsEnabled,flavors:desiredFlavors}).eq('id',productId);
      ...
      for(const file of pending){
        ...
        const {error:uploadError}=await db.storage.from('product-images').upload(path,file,{upsert:false,contentType:file.type});if(uploadError)throw uploadError;
        const {data:urlData}=db.storage.from('product-images').getPublicUrl(path);
        const {error:insertError}=await db.from('product_images').insert({product_id:productId,image_url:urlData?.publicUrl,storage_path:path,sort_order:sort++});
        if(insertError){await db.storage.from('product-images').remove([path]);throw insertError;}
      }
      if($('pOk'))$('pOk').textContent=desiredFlavorsEnabled&&desiredFlavors.length===0?'✓ Producto guardado. Atención: activaste sabores pero todavía no cargaste ninguno.':'✓ Producto, opciones y galería guardados.';
      await loadEnhancements();
    }catch(err){if($('pErr'))$('pErr').textContent='Producto guardado, pero faltó completar opciones/galería: '+(err.message||err);}
  });

  $('pDelete')?.addEventListener('click',()=>{
    const paths=currentGallery.map(x=>x.storage_path).filter(Boolean);if(!paths.length)return;
    setTimeout(async()=>{if(!$('productId')?.value){try{await db.storage.from('product-images').remove(paths);}catch(_){}}},500);
  });
