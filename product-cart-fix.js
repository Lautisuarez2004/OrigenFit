/* Origen Fit · garantiza ficha individual + carrito en cada tarjeta de Producto */
document.addEventListener('DOMContentLoaded',async()=>{
  ...
  let products=[];
  try{
    const {data,error}=await client.from('products').select('id,name,brand,stock,flavors_enabled,flavors,visible').eq('visible',true);
    if(error)throw error;products=data||[];
  }catch(err){console.warn('Ficha/carrito productos:',err?.message||err);return;}

  const hasFlavors=p=>!!p?.flavors_enabled&&Array.isArray(p?.flavors)&&p.flavors.some(x=>String(x||'').trim());
  const identify=card=>{
    const direct=products.find(p=>String(p.id)===String(card.dataset.productId||''));if(direct)return direct;
